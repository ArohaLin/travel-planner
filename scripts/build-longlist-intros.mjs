 ...
 *
 * 執行：node scripts/build-longlist-intros.mjs
...
async function aiIntros(items) {
  ...
  const prompt =
    '你是台東在地導覽。以下是台東的地點清單，請為每個地點寫一句「中性、客觀」的繁體中文簡介（20–30 字，描述它是什麼、特色），' +
...
const { rows } = await db.query(
  `SELECT id, name, category, google_place_id, rating_snapshot
    ...
    WHERE region='台東' AND tier='longlist'

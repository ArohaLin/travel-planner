...
export function TodoSheet({
  open, onClose, autoTodos, manualTodos, canEdit,
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4" style={{ paddingBottom: 'calc(32px + env(safe-area-inset-bottom))' }}>
          {unfinishedCount === 0 && done.length === 0 && (
          ...
          <div className="flex flex-col">
            {undone.map((item) => (
              <div key={item.id} className="flex items-center gap-3 py-2.5 border-b border-gray-100">
                {canEdit ? (
                  <button onClick={() => onToggleTodo(item.id, true)} aria-label="標記完成" className="flex-shrink-0">
                ...
                {editingId === item.id ? (
                  <input
                    ...
                    onBlur={() => { const v = editText.trim(); if (v && v !== item.title) onEditTodo(item.id, v); setEditingId(null) }}
                  ...
                  <span
                    onClick={() => { if (canEdit) { setEditingId(item.id); setEditText(item.title ?? '') } }}
                ...
                {canEdit && (
                  <button onClick={() => onDeleteTodo(item.id)} aria-label="刪除" className="flex-shrink-0 w-7 h-7 flex items-center justify-center text-gray-300 hover:text-red-500">
            ...
            {showDone && done.map((item) => (
              <div key={item.id} className="flex items-center gap-3 py-2.5 border-b border-gray-100">
                {canEdit ? (
                  <button onClick={() => onToggleTodo(item.id, false)} aria-label="取消完成" className="flex-shrink-0">
                ...
                <span className="flex-1 text-sm text-gray-400 line-through leading-snug">{item.title}</span>
                {canEdit && (
                  <button onClick={() => onDeleteTodo(item.id)} aria-label="刪除" className="flex-shrink-0 w-7 h-7 flex items-center justify-center text-gray-300 hover:text-red-500">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
          ...
          {canEdit && (
            <div className="flex items-center gap-2 mt-3">
              <input
                ...
                style={{ fontSize: '16px' }}
              />
              <button onClick={add} disabled={!newTitle.trim()} className="flex-shrink-0 px-3.5 py-2.5 rounded-xl bg-purple-500 text-white text-sm font-semibold disabled:opacity-40">新增</button>
            </div>
          )}

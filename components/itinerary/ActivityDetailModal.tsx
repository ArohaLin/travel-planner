...
export function ActivityDetailModal({ activity, dayNumber, onClose, canEdit, onEdit, onDelete, onAddNote, hasNote, onAssistantUpdate }: ActivityDetailModalProps) {
  ...
  // hero 照片：使用者自上傳優先，其次 Google Places photoRef
  const photoSrc = activity.userPhotoUrl ?? (activity.photoRef ? `/api/photo?ref=${encodeURIComponent(activity.photoRef)}` : null)

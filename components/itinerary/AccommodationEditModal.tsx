          ...
          {onUploadPhoto && (
            <div>
              ...
              <div className="flex items-center gap-3">
                <div className="w-20 h-20 rounded-2xl overflow-hidden bg-gray-100 flex-shrink-0 border border-gray-200 flex items-center justify-center">
                  {previewSrc ? (
                    ...
                    <img src={previewSrc} alt="住宿照片預覽" className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="flex-1 flex flex-col gap-1.5">
                  ...
                  {userPhotoUrl && !uploadingPhoto && (
                    <button type="button" onClick={() => setUserPhotoUrl(undefined)} className="text-xs text-gray-400 hover:text-red-500 self-start">
                      移除我的照片{accommodation.photoRef ? '（改用系統圖）' : ''}
                    </button>
                  )}
                </div>
              </div>
              ...
              {photoError
                ...
                : <p className="text-[11px] text-gray-400 mt-1">上傳後即可預覽；按下方「儲存」才正式套用。</p>}
            </div>
          )}

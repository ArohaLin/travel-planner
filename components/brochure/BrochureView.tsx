...
/**
 * 宣傳冊雜誌版面（純呈現、唯讀、無任何編輯）。
 * 設計：封面 → 旅程總覽 → 逐日章節（照片＋靜態路線圖＋介紹）→ 結尾。
 * ⚠️ 依需求不顯示任何金額（cost / 預算）；亦不顯示內部 notes / 預約連結 / 成員個資。
 */
...
function transportIcon(a: Activity): string {
  const s = `${a.transportMode ?? ''} ${a.title}`
  if (/船|渡輪|郵輪|遊艇/.test(s)) return '⛴'
  if (/飛機|航班|班機|機場/.test(s)) return '✈️'
  if (/高鐵|火車|台鐵|列車|鐵路/.test(s)) return '🚆'
  if (/捷運|地鐵/.test(s)) return '🚇'
  if (/巴士|客運|公車|遊覽車/.test(s)) return '🚌'
  if (/機車|摩托|騎車/.test(s)) return '🛵'
  if (/步行|散步|走/.test(s)) return '🚶'
  if (/自駕|開車|汽車|計程車|包車/.test(s)) return '🚗'
  return '🚗'
}
...
export function BrochureView({ itinerary, cache, token }: BrochureViewProps) {
  ...
  const photoUrl = (k: string) =>
    `/api/share/${token}/photo?k=${encodeURIComponent(k)}&v=${ver}`
    ...
    <article className="bg-white text-gray-800">
      ...
      <header className="relative min-h-[68vh] flex items-end overflow-hidden">
        ...
        <div className="relative w-full max-w-2xl mx-auto px-6 pb-12 text-white">
          <p className="text-sm tracking-[0.3em] uppercase opacity-80 mb-3">Travel Journal</p>
          <h1 className="font-serif text-4xl sm:text-5xl leading-tight mb-4">{metadata.title}</h1>
      ...
      <section className="max-w-2xl mx-auto px-6 py-10">
        <h2 className="font-serif text-2xl text-gray-900 mb-1">旅程總覽</h2>
        <div className="w-12 h-0.5 bg-purple-300 mb-6" />

        {metadata.style && metadata.style.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {metadata.style.map((s) => (
              <span key={s} className="text-xs px-3 py-1 rounded-full bg-purple-50 text-purple-600">
                {s}
              </span>
            ))}
          </div>
        ...
        <ol className="flex flex-col gap-2.5">
          {days.map((day) => (
            <li key={day.dayIndex} className="flex gap-3 items-baseline">
              <span className="font-serif text-purple-400 text-sm font-medium w-12 flex-shrink-0">
                Day {day.dayIndex + 1}
              </span>
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <div className="mb-6">
          <div className="flex items-baseline gap-3 mb-1">
            <span className="font-serif text-4xl text-purple-300 leading-none">
        ...
        <div className="flex flex-col gap-6">
          {acts.map((a) =>
            a.type === 'transport' ? (
              <TransportRow key={a.id} activity={a} />
            ) : (
              <ActivityBlock
                key={a.id}
                activity={a}
...
// ── 交通連接列（細、不放照片） ────────────────────────────
function TransportRow({ activity }: { activity: Activity }) {
  return (
    <div className="flex items-center gap-3 text-sm text-gray-400 pl-1">
      <span className="text-base">{transportIcon(activity)}</span>
...
function ActivityBlock({
  ...
  return (
    <div>
      <div className="rounded-2xl overflow-hidden shadow-sm mb-3 aspect-[16/10]">
        {hasPhoto(photoKey) ? (
      ...
      {a.highlight && (
        <p className="text-xs text-amber-600 mb-1">✦ {a.highlight}</p>
      )}

      {body && <p className="text-sm text-gray-600 leading-relaxed">{body}</p>}

      {a.foodItems && (
        <p className="text-sm text-gray-500 mt-1.5">🍽 {a.foodItems}</p>
      )}
      ...
      {a.tips && (
        <p className="text-xs text-gray-400 mt-1.5 leading-relaxed">💡 {a.tips}</p>
      )}
    </div>
  )
}

...
| AI 生成改用 responseSchema 鎖結構（取代 `patchParser` 正則 tag，降 parse 失敗） | rutugo, surmai, tripsage | 高/中 | 概念 | **進行中**（生成 L1 已上線；調整待做）|
...
- **2026-06-24｜AI 生成改 JSON 模式（責結構化 L1，先做「生成」）**：
  ...
  - 結論：L1 對「生成」零 UX 退化、可靠度上升、洗掉標籤抽取 hack。**下一步**：調整(adjust) 同樣上 L1（要決定串流取捨）＋整天替換簡化 schema；L2 只在需要時再 spike。⚠️ L1/L2 都修不了「截斷」（另案）。

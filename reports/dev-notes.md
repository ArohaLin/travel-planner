...
**可刪（多餘，低風險）**
- ~~`Activity.tags`~~ → **已刪（2026-06-28 P2-S1）**：0 寫入、僅 summaryRows 備援＋詳情裝飾 2 處讀，已移除。
- ~~`TravelLeg.midLat/midLng`~~ → **不可刪（盤點誤判）**：grep 確認 `route.ts:320-321` toPersistLegs 寫入、`ItineraryMap` 讀作距離標籤位置。**教訓：盤點「可刪」動手前必自己 grep 驗證。**
- ~~`TripMetadata.returnCity` → 刪~~ → **不可刪（2026-06-28 grep 複驗，盤點誤判）**：`route.ts:190-200` 用它畫地圖「終點」標記、`systemPrompt.ts:728-756` 餵進生成 prompt（路線/回程交通）。**又一個 midLat：文件「可刪」是錯的**。要精簡只能改成 `returnDate`、不能直接刪。
...
## F. 修 Bug 的流程規範（2026-06-30，從「兩次才修完」事件提煉）

> 事件：候車卡 geocode 用了錯誤 `day.city` 為 region bias，第一次只修「候車卡（rest+boardingPairId）」分支，忽略相鄰的「班次型交通卡（transport+boardingPairId）toLabel」分支，導致 Day 3 return journey 同樣壞掉，需第二次才修完。

### F-1 三個失誤層次

| 層次 | 失誤 | 正確做法 |
|---|---|---|
| **程式碼範圍** | 找到一個 `enqueue(…, day.city)` 就修它，沒搜尋同一檔案裡所有類似呼叫 | 修完目標點後，**grep 同一 function/概念的所有 call site**，逐一確認是否有同樣問題 |
| **DB 資料範圍** | 只清除被回報的那天（Day 0）的錯誤座標，沒掃全行程 | 修完 bug 後，**掃整份行程所有天**找同一來源的髒資料，一次清乾淨 |
| **心智模型範圍** | 正確推導「站名不需要 city bias」，卻只應用到候車卡（出發站），沒延伸到交通卡 toLabel（抵達站） | 找到規則時，**問「這個規則涵蓋哪些 code paths？」**，對稱地全部套用 |

### F-2 修 Bug 的標準流程（固定執行，不能省）

**Step 1 — 確認根因後，找所有同源 call site**
```bash
# 以 regionBias 問題為例：先找出所有用 day.city 作 regionBias 的 enqueue 呼叫
grep -n "enqueue" components/map/RoutePrefetcher.tsx  # 列出後逐行判斷
grep -n "enqueue" components/map/MapView.tsx
```
規則：**同一個 bug pattern 在同一個檔案裡出現幾次，全部一起修**，不只修被回報的那個。

**Step 2 — 掃描 DB，找同一來源的所有髒資料**

寫一個或重用腳本，搜尋整份（或所有）行程裡「可能受同樣 bug 影響」的資料：
```ts
// 例：找座標 ≈ (24.8046, 120.9718) 的所有活動（跨全部天）
for (const day of itin.days) {
  for (const a of day.activities) {
    if (Math.abs(a.location?.lat - WRONG_LAT) < 0.001) console.log(...)
  }
}
```
規則：**用報錯座標/值當 pattern 掃全部，不只清被回報的那筆**。

**Step 3 — 確認「概念是否對稱」**

問自己：
- 這個規則是 **單向** 還是 **雙向**？  
  （例：站名不需要 city bias → 出發站候車卡適用，**抵達站交通卡也適用**）
- 去程有的問題，回程（last day）是否也有？
- 有幾條 code path？修了幾條？

**Step 4 — 驗收不只看第一天，要看首尾**

確認至少 Day 0（出發）和 last day（返回）的地圖路線都正確，不只看被回報的那天。

### F-3 本次事件的正確流程（對照）

```
❌ 實際做的：
  發現 D0Wait00（候車卡）定位錯 → 修候車卡分支 → 清 Day0 → 部署
  → 使用者回報 Day3 也錯 → 才發現交通卡分支同樣有問題

✅ 正確應做的：
  發現 D0Wait00（候車卡）定位錯
  → Step 1: grep enqueue 全部呼叫 → 發現 transport+boardingPairId 也用 day.city → 一起修
  → Step 2: 掃全行程所有天，找座標 ≈ (24.8046, 120.9718) 的活動 → 發現 Day2 D2HSR001
  → Step 3: 「站名不需要 city bias」→ 出發站和抵達站都適用 → 兩個分支都改
  → 清 Day0 + Day2 的錯誤座標
  → 一次部署搞定
```
...
- **驗證限制**：authed 行程頁本機無法預覽 → 以「單元測試覆蓋核心邏輯＋tsc/build＋部署後煙霧測」為主；座標解析正確性**可用腳本打真實 Google Places API 驗**（見 `scripts/_verify-p1.mts`）；只有「登入後瀏覽器地圖/UI 視覺」要使用者實機。每階段獨立 commit/部署。**P3 不在本次。**

#### 進度狀態（2026-06-28）
- ✅ **P0 完成上線**（commit `6cd43f4`）：patchApplier 對序列/座標變的 op 主動清 `travelSig`；`DayView` 移動列過期退「直線概估・概估中」；新增 `lib/maps/estimateLeg`。驗證 `scripts/_verify-p0.mts` 12/12＋build。
- ✅ **P1 地基完成上線**：切片1 活動綁 `googlePlaceId`（`1007832`）；切片2 補座標優先 place_id（新增 `placeDetailsById`，`f387fd6`），真實 Google API `scripts/_verify-p1.mts` 4/4（都歷沙灘 place_id 解析座標精確、不誤抓綠島）。**使用者已實機驗 OK（刪卡概估、換地點定位）**。
- ✅ **P2 完成上線（2026-06-28，分 4 片）**：S1 刪 `tags`（`b29086d`；returnCity 經 grep 複驗為有用→不刪、更正 E-5）；S2 預約狀態統一三態 `reservationStatus`、`bookingRequired` 軟棄用＋`effectiveReservation` 後備讀＋修 summaryRows 孤島（`7aa7df0`）；S3 刪交通卡 `fromLabel`、`toLabel` 降後備（`555b4f3`）；S4 明確旗標 `hasPlace`/`isComposite` 取代四管線 rest 猜＋複合交通正則（`f552595`；新增 `lib/itinerary/activityFlags.ts` 單一來源、單元測 `scripts/_verify-p2-flags.mts` 8/8）。**全靠後備讀，舊資料免破壞性遷移**（編輯/AI patch 自然轉新欄位）。`transportMode`/`mealType` 既有欄位讀點本就優先（modeInfo/bucketMeals）、prompt 本就要求填，無需改。
- ❌ **P1 完善（S5）使用者決定不做（2026-06-28；已評估為高風險低報酬）**：①client geocode 改 place_id＝需 client PlacesService（複雜），且背景 `activityPhotos` 已用 place_id 精確補、邊際小；②`deriveDayCity` 改 place_id＝縣市現由 address 反推已可用、邊際小；③**`{0,0}`→null 三態收斂**＝`{0,0}` 是運作中的載重慣例（lenient parser 預設、5 處 hasCoords、dayCoords、ActivityCard 導航都依賴），改動橫跨 10+ 點且落在**座標最易出 bug 區**（issue-log 1-A/2-A/4-A），目前無對應 bug、純清理。**結論：P2 的實質（消連動斷裂/座標真相/雙軌/字串猜測）已由 P0+P1+S1~S4 達成；S5 屬技術債清理，建議有明確需求再做。**
- **若仍要做 S5**：先寫座標回歸測（涵蓋 {0,0}/null/有效三態 × 四管線），再逐點換、每點 build＋實機；動座標前必 grep（midLat 教訓）、正確性照 `_verify-p1.mts` 打真實 API 驗。

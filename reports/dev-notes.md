...
- ⏳ **P1 完善（待新階段）**：開地圖 client geocode（`MapView`/`RoutePrefetcher`）也優先 place_id、`deriveDayCity` 縣市改由 place_id 衍生、三態收斂廢 `{0,0}`。邊際遞減（主要同名問題切片2已解）。
- ⏳ **P2 全部（待新階段）**：見上方 P2 條與 E-5/E-6。每片牽動「AI 生成 prompt ＋ 全行程資料回填」，風險集中、需專注 context（故與 P1 完善一起另開階段做）。
- **新階段接續指令**：新對話說「**照 dev-notes E 做 P2（含 P1 完善）**」即可；先讀 E-0~E-7＋E-5/E-6＋issue-log 3-A~3-H/4-A/7。鐵則：**動任何座標/欄位前必先 grep 驗證**（`midLat` 誤判教訓）；座標正確性照 `_verify-p1.mts` 打真實 API 驗、UI 視覺請使用者實機。

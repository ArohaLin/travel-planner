...
### 3-G 編輯景點改名後，前面移動列殘留舊地名（顯示用過時 toLabel）

**發生時間**：2026-06-28

**症狀**：第 2 天下午衝浪行程地點從「都歷沙灘」改成「東河」，但前面那條移動列仍顯示「前往都歷沙灘」，沒自動更新。

**根本原因**：移動列「前往 X」的 X 有**兩個來源、不一致**——①**合成移動列**（無真實交通卡）即時讀「下一站 `placeLabel||title`」，會跟著改名更新；②**真實交通卡**的移動列用交通卡存在 DB 的 `toLabel`（`DayView.TravelRow` 第 137 行）。編輯景點走 `update_activity`（`patchApplier`）只改該活動欄位、**不重算交通卡 `toLabel`**，patch route 也不重算（`recomputeTimes` 只在刪除/重排/出發/排程時跑）→ 真實交通卡 `toLabel` 殘留舊名。與 **3-D**（刪除後交通卡 title 殘留）同類，但 3-D 是刪除路徑修的，**編輯改名路徑沒覆蓋**。

**修復方法**：移動列「前往 X」一律**即時讀時間軸下一站名**，不再依賴可能過時的 `toLabel`。`DayView` 真實交通卡的 `TravelRow` 也傳 `toName=（下一張非交通卡活動的 placeLabel||title）`；`TravelRow` 內 `to = toName?.trim() || transport.toLabel?.trim()`（即時優先、`toLabel` 後備）。與合成移動列的即時邏輯統一 → 改名後移動列即時正確、永不殘留。

**預防原則 / 待改進**：
- **顯示「下一站」一律即時讀時間軸鄰卡、不存快照**（與 3-D「同一資訊用同一來源」一致）。任何「會變的鄰卡資訊」都該即時算，不要依賴另一卡上存的副本。
- ⚠️ 殘留次要：交通卡**詳情視窗**顯示的是交通卡自身 `title`（如「開車前往都歷沙灘」），DB 未重算仍可能殘留舊名；移動列已不受影響。若要連詳情根治，需在 `update_activity` 套用後校正前置交通卡 `toLabel/title`（取捨：每次編輯多動前置卡＋多一筆歷程，暫不做）。
- **更深一層（同日續修）：時間/距離也會錯**。移動列的「約 N 分・X km」是用**座標**算的（`travelLegs`），與名字無關。`ActivityEditModal` 的座標只在改「地點/地址」欄才更新；只改「名稱」或「地點簡稱」**不動座標** → 換了地點但座標還停在舊點 → 時間/距離一直對應舊點、且不自動修。**修法**：`AddressAutocomplete` 選地點時回傳精確座標（原本丟棄 `place.geometry`）；`ActivityEditModal` 地點搜尋選取 → 同步更新 `placeLabel`＋`location`(精確座標)＋`address`，存檔直接用精確座標 → `travelLegs` 因座標改變自動重算 → 時間/距離正確。**教訓：顯示名與座標是兩套，UI 要讓「換地點」一個動作同時更新兩者**，否則使用者改了名字以為換了地點、路程其實還用舊座標。

### 3-H 移動列「只留 N 分・偏緊」誤判（沒算到「到達後 → 目的地開始」的空閒）

**發生時間**：2026-06-28

**症狀**：移動列顯示「路程約 22 分，只留 30 分，時間偏緊」，但目的地卡片 14:30 才開始、前面其實空很多，根本不該偏緊。

**根本原因**：「只留 N 分」（可用移動時間）對**真實交通卡**是用「**交通卡自己的時段** end−start」算的，**沒算到「交通卡結束 → 目的地活動開始」之間的空閒**。交通卡排得早、目的地較晚開始時，中間大段空閒被忽略 → `bufferStatus` 用偏小的可用時間判成偏緊。合成移動列（無交通卡）反而用「下一站 start − 前一站 end」、已涵蓋空閒、正確 → 兩者不一致。**三處同源**（移動列 `DayView`、橫幅 `bufferScan`、餵 AI 的 `buildTravelTimeSection`）都犯同錯（交通卡型用時段、合成型用到下一站）。

**修復方法**：三處統一——交通卡型可用時間改用「**下一站活動 start − 交通卡 start**」（涵蓋交通卡結束後到目的地開始的空閒）；合成型維持「下一站 start − 前一站 end」。`bufferStatus` 閾值不變。

**預防原則**：**「可用時間」一律算到「下一站活動開始」**——交通卡的時段是排程切出來的、不等於真正可用的緩衝。與 7-A/7-B 同理：buffer 判斷三處（移動列／橫幅／餵 AI）必須同一套算法。

### 3-I 每天最後的住宿卡時間倒退（顯示飯店 checkInTime，早於當天最後活動）

**發生時間**：2026-06-28

**症狀**：每天時間軸最底的住宿卡，時間比上一個活動還早（如 D3 最後活動 22:39，住宿卻顯示 15:00），看起來時序壞掉。

**根本原因**：`DayView` 住宿列 `RowFrame timeTop={day.accommodation.checkInTime}`——把**飯店政策「最早可入住時間」**（15:00/16:00）當成時間軸位置。但住宿排在當天所有活動之後，而最早可入住幾乎一定早於當晚行程結束（20:00 後）→ 時間軸跑到底突然倒退。混淆了「飯店幾點開放入住」與「你今晚幾點抵達」。

**修復方法**：住宿時間軸時間改算**當晚實際抵達/入住**＝`max(最後活動結束 + 到飯店車程, 飯店 checkInTime)`（不早於飯店可入住，也不早於當天行程結束）。最後活動是交通卡（如「返回民宿」）時車程為 0、直接用其結束時間。飯店「入住 15:00／退房 11:00」政策仍顯示在 `AccommodationCard` 內部（本就有）。純顯示層、不動資料。驗證 `scripts/_verify-acc-time.mts` 鏡像邏輯：台東行程 6 天全部新時間 ≥ 當天最後活動（D3 15:00→22:40、D5 15:00→21:54）。

**預防原則**：**政策時間 ≠ 時間軸時間**。時間軸位置一律用「實際發生/抵達時刻」，飯店/場館的營業政策時間只當資訊顯示在卡內，別拿來排序時間軸。
...
### A-1 上傳車票調整，多段交通被壓成一張卡、時間自己估

**發生時間**：2026-06-29

**症狀**：使用者在「調整」模式上傳台鐵＋高鐵車票照片、只打「幫我按照車票調整行程」。AI 文字回覆其實很到位（有抓到去程 22:21 抵達、回程 14:51 發車，甚至發現高鐵回程票方向疑似買反），但**實際存進去的交通卡品質差**：
- 去程「高鐵670 新竹→台北」＋「台鐵448 台北→花蓮」兩段被壓成**一張**「高鐵與普悠瑪前往花蓮 18:30~22:21」，起點 18:30 是估的（高鐵實際 18:57 發車），台北轉乘的 31 分空檔看不見，車次/座位/訂位代號全沒寫進 description。
- 回程同樣兩段壓成一張「候車與返回新竹 13:55~18:33」。

**根本原因**：調整／小幫手提示詞沒有「票券解析」專段，AI 預設會把轉乘型多段交通**合併成一張卡**、時間用估的而非照票面，車次資訊也不落到結構化欄位。不是模型不夠強，是沒教它規則。

**修復方法**：`lib/ai/systemPrompt.ts` 新增共用常數 `TICKET_TRANSPORT_RULES`（票券解析鐵則），注入 `buildAdjustPromptGemini`、`buildAssistantPrompt`、`buildAdjustPrompt` 三個產 patch 的提示詞。八條鐵則重點：① 多段逐 leg 拆成獨立 transport 卡，嚴禁合併 ② start/end 照票面一字不差、禁估算 ③ 轉乘空檔 ≥10 分要看得見（標「轉乘/候車」或排 rest 卡）④ 車次/座位/訂位代號寫進 tips ⑤ title 用「{工具}{車次} {起}→{迄}」⑥ 接駁段時間要趕得上票面發車 ⑦ reservationStatus 設 reserved ⑧ 票面與行程矛盾照票面排並在 message 提醒。
...
### A-3 AI 記憶與手動編輯衝突：請 AI 調整就把手動刪掉的東西又加回來

**發生時間**：2026-07-01（使用者回報「已發生多次、嚴重困擾」）

**症狀**：使用者手動刪除某活動（例：朝日溫泉夜泡）後，之後任何一次「請 AI 調整」（尤其「整天重新整理」這種大範圍改寫）就把它**原封不動加回來**——連時間（22:15–23:45）都與很久以前的舊記錄一字不差。

**根本原因（結構性、非提示詞沒寫好）**：`metadata.aiMemory`（單一自由文字欄）同時被拿來記兩種本質不同的東西——(A) 抽象偏好（不吃辣、步調慢）、(B) 具體行程決定（第3晚去泡朝日溫泉 22:15–23:45）。行程資料 `data.days[].activities` 本來就是「這趟有什麼」的**唯一真相**；記憶又抄一份 (B)，同一件事變兩份紀錄。而**手動新增/刪除/修改不經過 AI、不會同步記憶**（只有 adjust/consult 的 `<memory>` 標籤/JSON 欄位會寫回 aiMemory），兩份必然漂移 → 下次 AI 大範圍調整把記憶當權威，逐字「補回」使用者早就刪掉的項目。DB 鑑識：該條記憶自 2026-06-20 大重構寫入，中間使用者手動刪卡 4 次記憶都沒被清，直到某次「整天重構」被 AI 逐字抄回。

**修復方法（#48，2026-07-01）**：把記憶拆成兩塊、職責分明——
- **人工補充 `metadata.userNotes`**（新欄位）：使用者親手寫、**AI 唯讀不可改**。放「一定要／一定不要」的具體安排。因為只有使用者能改，永遠代表當下真實意圖、不漂移。`forPrompt` 不放進 JSON blob，改由 `buildUserNotesSection()` 加框呈現（唯讀・最高優先）。UI 在 `TripInfoCard`（讀取顯示＋編輯表單，琥珀色）。
- **AI 記憶 `metadata.aiMemory`**（收斂）：AI 可讀寫，但**只存抽象偏好、嚴禁存具體行程**。`buildMemorySection()` recap 明確禁止「因記憶提到某偏好就把具體景點加回行程」；`MEMORY_UPDATE_TAG_RULES`／各模式 JSON `memory` 欄位說明要求更新時只整併偏好、既有具體條目一律刪。
- `forPrompt` 同時把 `aiMemory`/`userNotes` 從 `<current_itinerary>` JSON 剝除，AI 只從加框段落讀到（避免讀到未加框的舊具體記憶當成資料）。四模式（adjust/gemini/consult/assistant）皆驗證兩塊正確渲染、JSON 無殘留。

**預防原則**：**行程資料是「行程內容」的唯一真相，任何會漂移的第二份副本都不可存具體行程**。長期記憶類欄位只存「行程資料表達不出來的抽象偏好」；使用者的具體硬性需求走「使用者親手維護、AI 唯讀」的獨立欄位，別讓 AI 自行推斷永久性。（遺留：上線前既有 aiMemory 可能仍含舊具體條目，需一次性清理或待 AI 依新規則自清。）
